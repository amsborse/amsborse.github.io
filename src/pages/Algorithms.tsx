...
const THEMES = {
  glassmorphism: {
    ...
    heading:
      "text-transparent bg-clip-text bg-gradient-to-r from-white via-[#e2e8f0] to-[#94a3b8] font-display font-semibold drop-shadow-sm",
    ...
    button:
      "bg-white/5 hover:bg-white/10 border border-white/10 rounded text-slate-300 font-mono transition-all",
    buttonActive:
      "bg-[var(--color-accent)]/10 border border-[var(--color-accent)]/30 text-[var(--color-accent)] rounded font-mono",
    buttonDanger:
      "bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 rounded font-mono transition-all",
    buttonStart:
      "bg-gradient-to-r from-[var(--color-accent)] to-[#4f46e5] hover:brightness-110 text-white rounded-lg shadow-[0_0_20px_rgba(56,189,248,0.25)] border border-white/10 font-mono uppercase tracking-wider",
    ...
    directorBoard:
      "bg-black/60 backdrop-blur-md border-[var(--color-accent)]/40 rounded-lg shadow-[0_10px_30px_rgba(0,0,0,0.5)]",
  ...
  "neo-brutalism": {
    ...
    heading:
      "text-black font-black uppercase tracking-tighter drop-shadow-[4px_4px_0_rgba(0,0,0,1)]",
    subheading:
      "text-xs font-bold uppercase text-black border-2 border-black px-2 py-1 bg-[#ff90e8] inline-block shadow-[2px_2px_0_rgba(0,0,0,1)]",
    ...
    button:
      "bg-white hover:bg-black hover:text-white border-2 border-black rounded-none text-black font-bold uppercase shadow-[4px_4px_0_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all",
    buttonActive:
      "bg-black text-white border-2 border-black rounded-none font-bold uppercase shadow-[4px_4px_0_rgba(0,0,0,1)]",
    buttonDanger:
      "bg-[#ff4949] hover:bg-black text-white hover:text-[#ff4949] border-2 border-black rounded-none font-bold uppercase shadow-[4px_4px_0_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all",
    buttonStart:
      "bg-[#ff90e8] hover:bg-black text-black hover:text-[#ff90e8] border-4 border-black rounded-none font-black uppercase text-lg shadow-[6px_6px_0_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all",
    ...
    directorBoard:
      "bg-[#ffeb3b] border-4 border-black rounded-none shadow-[8px_8px_0_rgba(0,0,0,1)]",
  ...
  synthwave: {
    ...
    heading:
      "text-transparent bg-clip-text bg-gradient-to-b from-[#22d3ee] to-[#d946ef] font-black tracking-widest drop-shadow-[0_0_15px_rgba(217,70,239,0.6)]",
    subheading:
      "text-[0.6rem] text-[#22d3ee] uppercase tracking-[0.4em] drop-shadow-[0_0_5px_rgba(34,211,238,0.8)]",
    panelBg:
      "bg-[#1a0b2e]/80 border-2 border-[#d946ef]/50 rounded-none shadow-[0_0_20px_rgba(217,70,239,0.2),inset_0_0_10px_rgba(217,70,239,0.1)] backdrop-blur-sm",
    canvasBg:
      "bg-[#05000a]/90 border-2 border-[#22d3ee]/50 rounded-none shadow-[inset_0_0_30px_rgba(34,211,238,0.1)] relative",
    dossierBg:
      "bg-[#1a0b2e]/90 border border-[#22d3ee]/50 rounded-none shadow-[0_0_20px_rgba(34,211,238,0.2)]",
    button:
      "bg-[#2e1065]/40 hover:bg-[#d946ef]/30 border border-[#d946ef]/50 text-[#fdf4ff] rounded-none shadow-[0_0_10px_rgba(217,70,239,0.3)] transition-all",
    buttonActive:
      "bg-[#22d3ee]/20 border-2 border-[#22d3ee] text-[#22d3ee] rounded-none shadow-[0_0_15px_rgba(34,211,238,0.5)]",
    buttonDanger:
      "bg-[#be123c]/40 hover:bg-[#be123c]/80 border border-[#f43f5e] text-[#fecdd3] rounded-none shadow-[0_0_15px_rgba(244,63,94,0.5)] transition-all",
    buttonStart:
      "bg-gradient-to-r from-[#d946ef] to-[#22d3ee] hover:brightness-125 text-white rounded-none shadow-[0_0_25px_rgba(217,70,239,0.6)] border-2 border-[#fdf4ff] font-black uppercase tracking-widest transition-all",
    ...
    directorBoard:
      "bg-[#1a0b2e]/90 border-2 border-[#22d3ee] rounded-none shadow-[0_0_30px_rgba(34,211,238,0.4)]",
  ...
  swiss: {
    ...
    button:
      "bg-transparent hover:bg-gray-100 border border-gray-300 text-black rounded-none transition-colors text-xs font-semibold uppercase tracking-wider",
    buttonActive:
      "bg-black text-white border border-black rounded-none text-xs font-semibold uppercase tracking-wider",
    buttonDanger:
      "bg-transparent hover:bg-red-600 hover:text-white hover:border-red-600 border border-gray-300 text-red-600 rounded-none transition-colors text-xs font-semibold uppercase tracking-wider",
    buttonStart:
      "bg-black hover:bg-gray-800 text-white border-none rounded-none text-sm font-bold uppercase tracking-widest transition-colors",
...
export default function Algorithms() {
      ...
      for (let j = 0; j < n - i - 1; j++) {
        ...
        if (tempArray[j] > tempArray[j + 1]) {
          ...
          if (visMode === "liquid-fill") {
            createSpill(j, tempArray[j]);
            createSpill(j + 1, tempArray[j + 1]);
          }
  ...
  const selectionSort = async (arr: number[], signal: AbortSignal) => {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      if (minIdx !== i) {
        ...
        if (visMode === "liquid-fill") {
          createSpill(i, tempArray[i]);
          createSpill(minIdx, tempArray[minIdx]);
        }
        const temp = tempArray[minIdx];
  ...
  const quickSort = async (arr: number[], signal: AbortSignal) => {
    ...
    const partition = async (low: number, high: number): Promise<number> => {
      ...
      if (visMode === "liquid-fill") {
        createSpill(i + 1, tempArray[i + 1]);
        createSpill(high, tempArray[high]);
      }
  ...
  const mergeSort = async (arr: number[], signal: AbortSignal) => {
    ...
    const merge = async (l: number, m: number, r: number) => {
      ...
      while (i < n1) {
        ...
        await executeFrame(`Copying remaining left elements.`, signal);
        tempArray[k] = L[i];
        setSwapIndices([k]);
        if (visMode === "liquid-fill") createSpill(k, tempArray[k]);
        setArray([...tempArray]);
      ...
      while (j < n2) {
        setActiveIndices([m + 1 + j]);
        await executeFrame(`Copying remaining right elements.`, signal);
        tempArray[k] = R[j];
        setSwapIndices([k]);
        if (visMode === "liquid-fill") createSpill(k, tempArray[k]);
        setArray([...tempArray]);
  ...
  const insertionSort = async (arr: number[], signal: AbortSignal) => {
    ...
    for (let i = 1; i < arr.length; i++) {
      ...
      let j = i - 1;
      setActiveIndices([i]);
      await executeFrame(`Inserting ${key} into sorted portion.`, signal);
      while (j >= 0 && tempArray[j] > key) {
        setSwapIndices([j, j + 1]);
        await executeFrame(`Moving ${tempArray[j]} to the right.`, signal);
  ...
  const heapSort = async (arr: number[], signal: AbortSignal) => {
    ...
    const heapify = async (n: number, i: number) => {
      ...
      if (l < n) {
        ...
        await executeFrame(`Heapify check left.`, signal);
        if (tempArray[l] > tempArray[largest]) largest = l;
      }
      if (r < n) {
        setActiveIndices([r, largest]);
        await executeFrame(`Heapify check right.`, signal);
        if (tempArray[r] > tempArray[largest]) largest = r;
      }
      if (largest !== i) {
        setSwapIndices([i, largest]);
        await executeFrame(`Heapify swap.`, signal);
        if (visMode === "liquid-fill") {
          createSpill(i, tempArray[i]);
          createSpill(largest, tempArray[largest]);
        }
        const swap = tempArray[i];
    ...
    for (let i = Math.floor(tempArray.length / 2) - 1; i >= 0; i--)
      await heapify(tempArray.length, i);
    for (let i = tempArray.length - 1; i > 0; i--) {
      setSwapIndices([0, i]);
      await executeFrame(`Extract max element to end.`, signal);
      if (visMode === "liquid-fill") {
        createSpill(0, tempArray[0]);
        createSpill(i, tempArray[i]);
      }
  ...
  const cocktailSort = async (arr: number[], signal: AbortSignal) => {
    ...
    while (swapped) {
      swapped = false;
      await executeFrame(`Forward pass.`, signal);
      for (let i = start; i < end; ++i) {
        setActiveIndices([i, i + 1]);
        await executeFrame(`Comparing ${tempArray[i]} and ${tempArray[i + 1]}.`, signal);
        if (tempArray[i] > tempArray[i + 1]) {
          setSwapIndices([i, i + 1]);
          await executeFrame(`Swapping.`, signal);
          if (visMode === "liquid-fill") {
            createSpill(i, tempArray[i]);
            createSpill(i + 1, tempArray[i + 1]);
          }
      ...
      await executeFrame(`Backward pass.`, signal);
    ...
    while (gap !== 1 || swapped === true) {
      ...
      for (let i = 0; i < n - gap; i++) {
        ...
        if (tempArray[i] > tempArray[i + gap]) {
          ...
          await executeFrame(`Swapping to clear turtles.`, signal);
          if (visMode === "liquid-fill") {
            createSpill(i, tempArray[i]);
            createSpill(i + gap, tempArray[i + gap]);
          }
          const temp = tempArray[i];
          tempArray[i] = tempArray[i + gap];
          tempArray[i + gap] = temp;
          setArray([...tempArray]);
          swapped = true;
        }
      }
  ...
  const gnomeSort = async (arr: number[], signal: AbortSignal) => {
    ...
    while (index < tempArray.length) {
      if (index === 0) index++;
      setActiveIndices([index, index - 1]);
      await executeFrame(`Gnome checks pots.`, signal);
      if (tempArray[index] >= tempArray[index - 1]) {
        await executeFrame(`Pots okay. Gnome steps forward.`, signal);
        index++;
      } else {
        setSwapIndices([index, index - 1]);
        await executeFrame(`Swapping pots and stepping back.`, signal);
        if (visMode === "liquid-fill") {
          createSpill(index, tempArray[index]);
          createSpill(index - 1, tempArray[index - 1]);
        }
  ...
  const oddEvenSort = async (arr: number[], signal: AbortSignal) => {
    ...
    while (!isSorted) {
      isSorted = true;
      await executeFrame(`Odd-indexed pass.`, signal);
      for (let i = 1; i <= tempArray.length - 2; i = i + 2) {
        setActiveIndices([i, i + 1]);
        await executeFrame(`Comparing odd-even.`, signal);
        if (tempArray[i] > tempArray[i + 1]) {
          setSwapIndices([i, i + 1]);
          await executeFrame(`Swapping pair.`, signal);
          if (visMode === "liquid-fill") {
            createSpill(i, tempArray[i]);
            createSpill(i + 1, tempArray[i + 1]);
          }
      ...
      for (let i = 0; i <= tempArray.length - 2; i = i + 2) {
        setActiveIndices([i, i + 1]);
        await executeFrame(`Comparing even-odd.`, signal);
        if (tempArray[i] > tempArray[i + 1]) {
          setSwapIndices([i, i + 1]);
          await executeFrame(`Swapping pair.`, signal);
          if (visMode === "liquid-fill") {
  ...
  const radixSort = async (arr: number[], signal: AbortSignal) => {
    ...
    for (let exp = 1; Math.floor(max / exp) > 0; exp *= 10) {
      ...
      const output = new Array(tempArray.length).fill(0);
      const count = new Array(10).fill(0);
      for (let i = 0; i < tempArray.length; i++) {
        setActiveIndices([i]);
        await executeFrame(`Checking ${exp}s digit of ${tempArray[i]}.`, signal);
        count[Math.floor(tempArray[i] / exp) % 10]++;
      }
      for (let i = 1; i < 10; i++) count[i] += count[i - 1];
      for (let i = tempArray.length - 1; i >= 0; i--) {
        const digit = Math.floor(tempArray[i] / exp) % 10;
        output[count[digit] - 1] = tempArray[i];
        count[digit]--;
      }
      await executeFrame(`Rebuilding array from buckets.`, signal);
      for (let i = 0; i < tempArray.length; i++) {
        setActiveIndices([i]);
        await executeFrame(`Placing ${output[i]}.`, signal);
        tempArray[i] = output[i];
        setSwapIndices([i]);
        if (visMode === "liquid-fill") createSpill(i, tempArray[i]);
        setArray([...tempArray]);
      }
    }
  };
    ...
    try {
      switch (sortMethod) {
        ...
        case "selection":
          ...
          break;
        case "quick":
          await quickSort(array, signal);
          break;
        case "insertion":
          await insertionSort(array, signal);
          break;
        case "merge":
          await mergeSort(array, signal);
          break;
        case "heap":
          await heapSort(array, signal);
          break;
        case "shell":
          await shellSort(array, signal);
          break;
        case "cocktail":
          await cocktailSort(array, signal);
          break;
        case "comb":
          await combSort(array, signal);
          break;
        case "gnome":
          await gnomeSort(array, signal);
          break;
        case "odd-even":
          await oddEvenSort(array, signal);
          break;
        case "radix":
          await radixSort(array, signal);
          break;
      }
  ...
  const renderTooltip = (val: number) => (
    <div
      className={`absolute bottom-full mb-1 left-1/2 -translate-x-1/2 text-[10px] px-1.5 py-0.5 opacity-0 group-hover:opacity-100 transition-opacity z-50 pointer-events-none whitespace-nowrap font-mono ${theme.buttonActive}`}
    >
      {val}
    </div>
  );
      ...
      return (
        <div className="flex items-end justify-center w-full h-full gap-[4px] mt-12 pb-2 relative">
          {array.map((val, i) => {
            ...
            const containerStyle =
              appTheme === "glassmorphism"
                ? "rounded-b-full border border-t-0 border-white/30 bg-white/5 shadow-[inset_0_0_8px_rgba(255,255,255,0.2)]"
                : appTheme === "neo-brutalism"
                  ? "border-2 border-t-0 border-black bg-white rounded-none"
                  : appTheme === "synthwave"
                    ? "border border-t-0 border-cyan-500/50 bg-[#1a0b2e]/50 shadow-[inset_0_0_15px_rgba(34,211,238,0.2)] rounded-none"
                    : "border border-t-0 border-gray-300 bg-white rounded-none";

            const liquidShadow =
              appTheme === "glassmorphism"
                ? `inset 0 -10px 10px rgba(0,0,0,0.2), 0 0 15px ${shadowColor}`
                : appTheme === "synthwave"
                  ? `0 0 20px ${shadowColor}, inset 0 0 10px ${color}`
                  : "none";

            return (
              <div
                key={i}
                className={`flex-1 h-full relative overflow-hidden flex items-end group cursor-crosshair transition-colors ${containerStyle}`}
              >
                {appTheme === "glassmorphism" && (
                  <div className="absolute top-2 bottom-4 left-[15%] w-[15%] bg-gradient-to-b from-white/40 to-transparent rounded-full z-20 pointer-events-none blur-[0.5px]"></div>
                )}
                <div
                ...
                >
                  {appTheme === "glassmorphism" && (
                    <div className="absolute top-0 left-0 w-full h-[3px] bg-white/60 blur-[1px] rounded-[50%]"></div>
                  )}
                  {appTheme === "neo-brutalism" && (
                    <div className="absolute top-0 left-0 w-full h-[2px] bg-black"></div>
                  )}
                  {appTheme === "synthwave" && (
                    <div className="absolute top-0 left-0 w-full h-[2px] bg-white/80 shadow-[0_0_5px_#fff]"></div>
                  )}
                </div>
                {renderTooltip(val)}
              </div>
          ...
          {spills.map((spill) => (
            ...
            >
              <div
                style={{ backgroundColor: theme.spillColor, boxShadow: theme.spillShadow }}
                className={`w-[4px] h-[8px] ${appTheme === "neo-brutalism" || appTheme === "swiss" ? "rounded-none border border-black" : "rounded-full blur-[0.5px]"}`}
              />
            </div>
          ))}
      ...
      return (
        <div className="relative w-full h-full mt-12 pb-8">
          {array.map((val, i) => {
            ...
            const bg =
              appTheme === "glassmorphism" ? `radial-gradient(circle at 30% 30%, #fff, ${c})` : c;
            const orbShadow =
              appTheme === "glassmorphism"
                ? isPulsing
                  ? `0 0 20px 4px ${shadow}`
        ...
        <div className="relative w-full h-full mt-12 pb-8">
          {array.map((val, i) => {
            ...
            return (
              ...
              >
                <div
                  style={{ backgroundColor: c, boxShadow: theme.getBarShadow(c, c) }}
                  className={`w-2 h-2 z-10 hover:scale-150 transition-transform ${appTheme === "neo-brutalism" || appTheme === "swiss" ? "rounded-none border border-black" : "rounded-full"}`}
                />
                <div
                  style={{
                    background:
                      appTheme === "neo-brutalism" || appTheme === "swiss"
                        ? c
                        : `linear-gradient(to bottom, ${c}, transparent)`,
                    height: isDefault ? "15px" : "50px",
                    opacity: isDefault ? 0.3 : 0.9,
                    filter:
                      appTheme === "neo-brutalism" || appTheme === "swiss" ? "none" : "blur(1px)",
  ...
  return (
    <>
      ...
      <div
        className={`min-h-screen relative overflow-x-hidden pb-32 pt-20 transition-colors duration-500 ${theme.pageBg}`}
      >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-start mb-12">
            ...
            <div
              className={`w-full lg:w-80 p-6 flex flex-col gap-6 shrink-0 z-20 ${theme.panelBg}`}
            >
              ...
              <div>
                <span className={`block mb-4 flex justify-between items-center ${theme.textMuted}`}>
                  Execution Mode
                  {isStepMode && (
                    <span className="bg-red-500 text-white px-1.5 py-0.5 text-[10px] uppercase font-bold animate-pulse">
                      EDU
                    </span>
                  )}
                </span>
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={isSorting}
                    className={`flex-1 py-2 ${!isStepMode ? theme.buttonActive : theme.button} disabled:opacity-50 disabled:cursor-not-allowed`}
                  >
                    Auto-Run
                  </button>
                  <button
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 px-4 py-2.5 ${theme.button}`}
                  >
                    Generate
                  </button>
                  <button
                    onClick={handleReset}
                    className={`px-4 py-2.5 ${theme.buttonDanger}`}
              ...
              {/* Sliders */}
              <div
                className={`flex flex-col gap-5 transition-opacity ${isStepMode ? "opacity-30 pointer-events-none" : "opacity-100"}`}
              >
                <div>
                  <div className="flex justify-between mb-2">
                    <label className={theme.textMuted}>Array Size</label>
                    <span
                      className={`text-xs font-bold ${appTheme === "glassmorphism" ? "text-[var(--color-accent)]" : "text-current"}`}
                    >
                      {arraySize}
                    </span>
                  </div>
                  <input
                    ...
                    className={`w-full opacity-80 hover:opacity-100 disabled:opacity-30 cursor-pointer ${theme.inputSlider}`}
                  />
                </div>
                <div>
                  <div className="flex justify-between mb-2">
              ...
              <button
                onClick={startSort}
                disabled={isSorting}
                className={`mt-2 px-6 py-4 disabled:opacity-50 disabled:pointer-events-none ${theme.buttonStart}`}
              >
            ...
            <div
              className={`flex-1 w-full p-6 flex flex-col justify-end min-h-[500px] h-[60vh] lg:h-[75vh] relative overflow-hidden ${theme.canvasBg}`}
            >
              {/* Educational Mode Director's Board Overlay */}
              {isStepMode && explanationText && (
                <div
                  className={`absolute top-6 left-1/2 -translate-x-1/2 z-30 w-11/12 max-w-lg p-6 flex flex-col items-center gap-4 animate-fade-in ${theme.directorBoard}`}
                >
                  <p className={`text-sm text-center leading-relaxed ${theme.textBase}`}>
                    <strong className={`text-xs uppercase block mb-1 ${theme.textMuted}`}>
                      Current Step
                    </strong>
                    {explanationText}
                  </p>
                  {isWaitingForNext ? (
                    <button
                      ...
                      className={`px-6 py-2 mt-2 animate-pulse hover:animate-none ${theme.buttonActive}`}
                    >
                      Next Step
                    </button>
                  ) : (
                    <div className="h-8 flex items-center justify-center mt-2">
                      <span className="flex gap-2">
                        <span
                          className="w-2 h-2 bg-current opacity-50 rounded-full animate-bounce"
          ...
          <div
            className="w-full mb-24 animate-fade-in transition-opacity duration-500"
            key={sortMethod}
          >
            <div
              className={`w-full p-8 md:p-12 flex flex-col lg:flex-row gap-12 ${theme.dossierBg}`}
            >
              {/* Left Column: Story */}
              <div className="flex-1">
                <div className="mb-6">
                  <span className={theme.subheading}>Algorithm Dossier</span>
                  <h2 className={`text-3xl md:text-4xl mb-2 ${theme.heading}`}>
                    {currentAlgoData.name}
                  </h2>
                  <p className={theme.textMuted}>
                    Invented by: <span className={theme.textBase}>{currentAlgoData.inventor}</span>{" "}
                    • {currentAlgoData.year}
                  </p>
                </div>
                ...
                <div className={`p-6 ${theme.panelBg} !shadow-none !border-current opacity-90`}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <span
                        className={`text-lg font-bold ${appTheme === "neo-brutalism" || appTheme === "swiss" ? "text-black" : "text-[#10b981]"}`}
                      >
                        {currentAlgoData.complexities.best}
                      </span>
                    </div>
                    <div>
                      <span className={`block mb-1 ${theme.textMuted}`}>Average Time</span>
                      <span
                        className={`text-lg font-bold ${appTheme === "neo-brutalism" || appTheme === "swiss" ? "text-black" : "text-[#f59e0b]"}`}
                      >
                        {currentAlgoData.complexities.average}
                      </span>
                    </div>
                    <div>
                      <span className={`block mb-1 ${theme.textMuted}`}>Worst Time</span>
                      <span
                        className={`text-lg font-bold ${appTheme === "neo-brutalism" || appTheme === "swiss" ? "text-black" : "text-[#ef4444]"}`}
                      >
                        {currentAlgoData.complexities.worst}
                      </span>
                    </div>
                    <div>
                      <span className={`block mb-1 ${theme.textMuted}`}>Space</span>
                      <span
                        className={`text-lg font-bold ${appTheme === "neo-brutalism" || appTheme === "swiss" ? "text-black" : "text-[#3b82f6]"}`}
                      >
                        {currentAlgoData.complexities.space}
                      </span>
                    </div>
                  </div>
                </div>
