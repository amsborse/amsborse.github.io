    ...
    <>
      ...
      <div className="writing-index article-shell max-w-3xl py-16 sm:py-20 lg:py-24">
        ...
        <p className="mt-6 max-w-xl text-[1.0625rem] leading-[1.75] text-[var(--color-body)]">
          ...
          <code className="rounded bg-white/[0.05] px-1.5 py-0.5 font-mono text-[0.85em] text-[var(--color-ink-soft)]">
            {contentPaths.posts}
          </code>
          ; order is set in{" "}
          <code className="rounded bg-white/[0.05] px-1.5 py-0.5 font-mono text-[0.85em] text-[var(--color-ink-soft)]">
          ...
          <ul className="mt-6 divide-y divide-[var(--color-border)] border-t border-[var(--color-border)]">
            {featured.map((post) => (
              <li key={post.slug}>
                ...
                >
                  <h3 className="font-display text-xl font-medium leading-snug text-[var(--color-ink)] group-hover:text-[var(--color-ink-soft)]">
                    {post.title}
                  </h3>
          ...
          <ul className="mt-6 divide-y divide-[var(--color-border)] border-t border-[var(--color-border)]">
            {all.map((post) => (
              <li key={post.slug}>
                ...
                >
                  <span className="font-medium text-[var(--color-ink)] group-hover:text-[var(--color-ink-soft)]">
                    {post.title}
                  </span>
