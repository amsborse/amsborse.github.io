...
const TOPICS: LearningTopic[] = [
  {
    ...
    description: "Interactive visual simulation of sorting algorithms, execution paths, and spatial telemetry. Bringing abstract data mechanics into a responsive physical dimension.",
    icon: "⚡",
    path: "/algorithm",
    status: "active",
    ...
    renderPortalVisual: () => (
      ...
      <svg className="w-full h-full p-4" viewBox="0 0 200 200" fill="none">
        <g transform="translate(10, 30)">
          {/* Base plane */}
          <path d="M 0 100 L 90 55 L 180 100 L 90 145 Z" fill="rgba(99, 102, 241, 0.05)" stroke="rgba(99, 102, 241, 0.2)" strokeWidth="1" />
  ...
  },
  {
    ...
    description: "Visual sandboxes for load balancing policies, message queues handling high TPS, rate-limiting algorithms, and consensus protocols in distributed networks.",
    icon: "🌐",
    status: "coming-soon",
    ...
    renderPortalVisual: () => (
      ...
      <svg className="w-full h-full p-4" viewBox="0 0 200 200" fill="none">
        {/* Paths */}
        <path d="M 40 100 L 100 50 M 40 100 L 100 150 M 100 50 L 160 100 M 100 150 L 160 100 M 100 50 L 100 150" stroke="rgba(16, 185, 129, 0.25)" strokeWidth="1.5" strokeDasharray="4 4" />
        
        {/* Glowing Node 1 */}
        <circle cx="40" cy="100" r="8" fill="#0d0e15" stroke="#10b981" strokeWidth="2.5" />
        <circle cx="40" cy="100" r="14" stroke="#10b981" strokeWidth="1" opacity="0.3" className="animate-ping" style={{ transformOrigin: '40px 100px' }} />
  ...
  },
  {
    ...
    description: "Interactive exploration of agentic AI guardrails, risk-scoring engines, tokenized authorization, and Purview-style zero-touch enforcement models.",
    icon: "🛡️",
    status: "coming-soon",
  ...
  },
  {
    ...
    description: "Interactive guide on vector embedding projections, similarity search metrics, and Retrieval-Augmented Generation context window injection.",
    icon: "🧠",
    status: "coming-soon",
    ...
    renderPortalVisual: () => (
      ...
      <svg className="w-full h-full p-4" viewBox="0 0 200 200" fill="none">
        ...
        <circle cx="100" cy="100" r="25" stroke="rgba(251,191,36,0.25)" strokeWidth="1" strokeDasharray="3 3" />
      ...
      <div className="w-full shrink-0 group-hover:w-[60%] transition-all duration-500 ease-out flex flex-col justify-between h-full p-8 pr-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {topic.tags.map((tag) => (
              <span key={tag} className="text-[9px] font-mono text-slate-500 bg-white/5 border border-white/5 px-2 py-0.5 rounded">
                {tag}
              </span>
            ))}
          </div>
      ...
      */}
      <div 
        ...
        style={{
          ...
          maskImage: "radial-gradient(circle 120px at var(--mouse-x) var(--mouse-y), black 20%, transparent 100%)",
          WebkitMaskImage: "radial-gradient(circle 120px at var(--mouse-x) var(--mouse-y), black 20%, transparent 100%)",
        }}
      />

      {/* Subtle top color gradient line */}
      <div className={`absolute top-0 inset-x-0 h-[2px] bg-gradient-to-r ${topic.color} opacity-70 group-hover:opacity-100 transition-opacity duration-300`} />
      ...
      <div className="min-h-screen relative overflow-x-hidden pb-32 pt-20 bg-transparent text-[#f1f3f7] font-sans">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
          ...
          <header className="text-center mb-16">
            ...
            >
              Explore complex concepts made tangible through interactive simulations. Move values, tweak parameters, and analyze real-time telemetry.
            </motion.p>
          </header>
